...
# tabfm <img src="man/figures/logo.svg" align="right" height="139" alt="tabfm hex sticker" />
...
familiar R modelling interface: a fitting function plus `predict()`.
...
first `tabfm()` call per task pays the load cost.

## Limitations

- Predictions require a working Python installation; `install_tabfm()`
  sets everything up via reticulate.
- The first `tabfm()` call downloads the pretrained weights from the
  Hugging Face Hub (cached afterwards in `~/.cache/huggingface/`). You
  may see a warning about unauthenticated requests; it’s harmless, but
  you can silence it and get faster downloads by putting a free Hugging
  Face [access token](https://huggingface.co/settings/tokens) in your
